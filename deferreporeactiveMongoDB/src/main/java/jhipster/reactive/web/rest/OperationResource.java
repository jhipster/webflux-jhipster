...
public class OperationResource {
    ...
    public Mono<ResponseEntity<Operation>> createOperation(@Valid @RequestBody Operation operation) throws URISyntaxException {
        ...
        return result.flatMap((Operation savedOperation) ->
            Mono.defer(() ->
                Mono.just(ResponseEntity.created(URI.create("/api/operations/" + savedOperation.getId()))
        ...
        return operationRepository.findById(id)
            .flatMap(savedOperation ->
                Mono.defer(() -> {
                    ...
                    return Mono.just(ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id)).build());
                }));
    }
}
