...
public class LabelResource {
    ...
    public Mono<ResponseEntity<Label>> createLabel(@Valid @RequestBody Label label){
        ...
        return result.map((Label savedLabel)->{
            try {
                return ResponseEntity.created(new URI("/api/labels/" + savedLabel.getId()))
                    .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, savedLabel.getId().toString()))
                    .body(savedLabel);
    ...
    public Mono<ResponseEntity<Label>> getLabel(@PathVariable String id) {
        ...
        Mono<Label> label = labelRepository.findById(UUID.fromString(id));
    ...
    public Mono<ResponseEntity<Void>> deleteLabel(@PathVariable String id) {
        ...
        return labelRepository.findById(UUID.fromString(id))
            .map(savedLabel -> {
                labelRepository.deleteById(UUID.fromString(id)).subscribe();
