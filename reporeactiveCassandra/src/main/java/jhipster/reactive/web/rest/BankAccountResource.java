...
public class BankAccountResource {
    ...
    public Mono<ResponseEntity<BankAccount>> createBankAccount(@Valid @RequestBody BankAccount bankAccount) throws URISyntaxException {
        ...
        return result.map((BankAccount savedBankAccount)->{
            try{
                return ResponseEntity.created(new URI("/api/bank-accounts/" + savedBankAccount.getId()))
                    .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, savedBankAccount.getId().toString()))
    ...
    public Mono<ResponseEntity<BankAccount>> updateBankAccount(@Valid @RequestBody BankAccount bankAccount) throws URISyntaxException {
        ...
        return result.map((BankAccount savedBankAccount)->
            ResponseEntity.ok()
                .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, savedBankAccount.getId().toString()))
                .body(savedBankAccount)
        );
    }
    ...
    public Mono<ResponseEntity<BankAccount>> getBankAccount(@PathVariable String id) {
        ...
        Mono<BankAccount> bankAccount = bankAccountRepository.findById(UUID.fromString(id));
    ...
    public Mono<ResponseEntity<Void>> deleteBankAccount(@PathVariable String id) {
        ...
        return bankAccountRepository.findById(UUID.fromString(id)).
            map(savedBankAccount -> {
                bankAccountRepository.deleteById(UUID.fromString(id)).subscribe();
