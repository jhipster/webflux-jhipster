...
public class OperationResource {
    ...
    public Mono<ResponseEntity<Operation>> createOperation(@Valid @RequestBody Operation operation) throws URISyntaxException {
        ...
        return result.map((Operation savedOperation) -> {
            try {
                return ResponseEntity.created(new URI("/api/labels/" + savedOperation.getId()))
                    .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, savedOperation.getId().toString()))
                    .body(savedOperation);
    ...
    @Timed
    public Flux<Operation> getAllOperations() {
    ...
    public Mono<ResponseEntity<Operation>> getOperation(@PathVariable String id) {
        ...
        Mono<Operation> operation = operationRepository.findById(UUID.fromString(id));
    ...
    public Mono<ResponseEntity<Void>> deleteOperation(@PathVariable String id) {
        ...
        return operationRepository.findById(UUID.fromString(id)).
            map(savedOperation -> {
                operationRepository.deleteById(UUID.fromString(id)).subscribe();
